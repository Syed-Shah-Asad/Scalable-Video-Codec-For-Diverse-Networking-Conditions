...
void inverse_quantizer(unsigned char *frame, int QP);
...
int main(int argc, char *argv[])
{
	...
	for(no=0; no<atoi(argv[2]); no++){//for 10 frames only
		...
		inverse_quantizer(wire,atoi(argv[1]));
	...
	for(y = 0; y < HEIGHT; y+=BLOCK_H) {
		for(x = 0; x < WIDTH; x+= BLOCK_W) {
			...
			for (i = 0; i < 8; i++ )
			{
				for (j = 0; j < 8; j++ )
				{
					coeff[i][j] = ceil (coeff[i][j]-0.5);
					coeff[i][j] = ( coeff[i][j] * QP);
				}
			}
			for(i = 0; i < BLOCK_H; i++) {
				for(j = 0; j < BLOCK_W; j++) {
					frame[(x+j)+(WIDTH*(y+i))]=(signed char)coeff[i][j];
				}
			}
		}
	}
